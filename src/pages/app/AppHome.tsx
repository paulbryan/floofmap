...
const calculateStreak = (
  walks: WalkBase[]
): { streak: number; walkedToday: boolean } => {
  ...
  if (
    !isEqual(mostRecentWalkDay, today) &&
    !isEqual(mostRecentWalkDay, yesterday)
  ) {
    ...
    const fetchData = async () => {
      ...
      const [
        ...
        supabase
          .from('walks')
          .select(
            'id, started_at, distance_m, duration_s, sniff_time_s, dogs(id, name, avatar_url)'
          )
          .eq('user_id', user.id)
      ...
      const displayName =
        profileResult.data?.full_name || user.email?.split('@')[0] || ''
      ...
      if (walks) {
        ...
        const totalDistance = allWalks.reduce(
          (sum, w) => sum + (w.distance_m || 0),
          0
        )
        const totalSniffTime = allWalks.reduce(
          (sum, w) => sum + (w.sniff_time_s || 0),
          0
        )
        const avgSniffs =
          allWalks.length > 0
            ? Math.round(totalSniffTime / allWalks.length / 60)
            : 0
        ...
        <div className='max-w-6xl mx-auto'>
          ...
          {weather?.forecast && weather.forecast.length > 0 && (
            <motion.div
              ...
              className='flex gap-2 overflow-x-auto pb-2 mb-4 scrollbar-hide w-full max-w-xs sm:max-w-md md:max-w-lg lg:max-w-2xl'
            >
            ...
            >
              <div className='text-center max-w-lg mx-auto'>
                ...
                <h2 className='text-xl lg:text-2xl font-bold mb-2'>
                  Welcome to Floof Map!
                </h2>
                <p className='text-muted-foreground mb-6'>
                  Track your dog walks, discover sniff-worthy spots, and explore
                  dog-friendly places in your neighborhood.
                </p>
                  ...
                  <div className='flex-1 min-w-0'>
                    <div className='flex items-center gap-2'>
                      <p className='font-medium truncate'>
                        {formatWalkDate(walk.started_at)}
                      </p>
                      {walk.dogs && (
                    ...
                    <p className='text-sm text-muted-foreground'>
                      {formatDistance(walk.distance_m)} •{' '}
                      {formatDuration(walk.duration_s)}
                    </p>
                  </div>
                  <div className='text-right shrink-0'>
                    <p className='font-semibold text-primary'>
                      {Math.round((walk.sniff_time_s || 0) / 60)}
                    </p>
